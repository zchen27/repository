...
public class ImportListProcessingActor extends UntypedActor
{
	...
	static public Props props(List<Integer> list, ActorRef listManager,
			ActorRef replyTo)
	{
		return Props.create(ImportListProcessingActor.class, list, listManager,
				replyTo);
	}
	...
	public ImportListProcessingActor(List<Integer> list, ActorRef listManager,
			ActorRef replyTo)
	{
		this.list = list;
	...
	public void onReceive(Object msg) throws Exception
	{
		...
		else if (msg instanceof NullActorResultMessage)
		{
			...
			if (iterator.hasPrevious())
			{
				...
				listManager.tell(new ConsActorRequestMessage(head, nullActor),
						getSelf());
			}
			else
			{ // Construction complete!
				replyTo.tell(new ImportListResultMessage(nullActor),
						listManager);
		...
		else if (msg instanceof ConsActorResultMessage)
		{
			...
			{
				Integer head = iterator.previous();
				listManager.tell(new ConsActorRequestMessage(head, consActor),
						getSelf());
			}
			else
			{ // Construction complete!
				replyTo.tell(new ImportListResultMessage(consActor),
						listManager);
				getContext().stop(getSelf());
			}
		}
		...
		{
			System.out.printf("Bad message to ImportListProcessingActor:  %s%n",
					msg.toString());
			unhandled(msg);
		}
	}
