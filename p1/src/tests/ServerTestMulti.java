...
public class ServerTestMulti
{
	...
	public void setUp() throws Exception
	{
		...
		Constructor<ServerPrinter> serverConstructor = ServerPrinter.class
				.getDeclaredConstructor((Class<ServerPrinter>[]) null);
		...
		serverInstance.set(null,
				serverConstructor.newInstance((Object[]) null));
	}
	...
	private void testMultiThread(int testNumber, int nrSellers, int nrBuyers)
	{
		...
		for (int i = 0; i < nrSellers; ++i)
		{
			...
			sellers[i] = new Seller(/* server = */ serverPrinter,
					/* sellerName = */ "Seller" + i, /* cycles = */ 100,
					/* maxSleepTimeMs = */ 2, /* randomSeed = */ i);
		...
		{
			buyers[i] = new Bidder(/* server = */ AuctionServer.getInstance(),
					/* buyerName = */ "Buyer" + i, /* initialCash = */ 1000,
					/* cycles = */ 20, /* maxSleepTimeMs = */ 2,
					/* randomSeed = */ i);
		...
		// try
		// {
		// // Output the log of the test in a separate file, for ease of
		// debugging.
		...
		assertFalse("You don't have any recording of how much was spent.",
				moneySpent == 0);
		assertTrue(
				"For test number " + testNumber + " the server revenue "
						+ AuctionServer.getInstance().revenue()
						+ " differs from the revenue reported by the buyers "
						+ moneySpent + "!",
				moneySpent == AuctionServer.getInstance().revenue());
		for (int i = 0; i < buyers.length; ++i)
		{
			assertTrue("For test number " + testNumber
					+ " the server items capacity "
					+ buyers[i].mostItemsAvailable() + " exceeds the limit of "
					+ AuctionServer.serverCapacity + "!",
					buyers[i]
							.mostItemsAvailable() <= AuctionServer.serverCapacity);
		}
	}
