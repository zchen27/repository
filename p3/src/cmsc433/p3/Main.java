...
public class Main
{
	...
	public void solve()
	{
		...
		MazeSolver[] solvers = { //new STMazeSolverRec(maze),
				new STMazeSolverDFS(maze), //new STMazeSolverBFS(maze),
		...
		for (MazeSolver solver : solvers)
		{
			...
			if (soln == null)
			{
				if (!solvable) System.out.println(
						"Correctly found no solution in " + sec + " seconds.");
				else System.out.println(
						"Incorrectly returned no solution when there is one.");
			}
			else
			{
				if (maze.checkSolution(soln)) System.out.println(
						"Correct solution found in " + sec + " seconds.");
	...
	public static void main(String[] args)
	{
		 ...
		 * "    To solve the maze stored in filename."); System.exit(-1); }
		 */

		// These lines are to run via Eclipse without a command prompt
		String mazeLocationNotInProjectFolder = ""; // replace
																	 // this
																	 // with
																	 // your
																	 // maze
		...
		String[] replaceArgs = {
				mazeLocationNotInProjectFolder + whichMazeToUse };
		args = replaceArgs;
		...
		{
			System.out
					.println("ClassNotFoundException while reading maze from: "
							+ args[0]);
			e.printStackTrace();
		}
		catch (IOException e)
		{
			System.out
					.println("IOException while reading maze from: " + args[0]);
			e.printStackTrace();
		}
	...
	{
		MazeInputStream in = new MazeInputStream(
	...
	private void initDisplay()
	{
		...
		if (false)
		{
			...
			frame.setSize(
					maze_width * cell_size + insets.left + insets.right + 3,
