...
public class MazeDisplay extends JPanel
{
	...
	Hashtable<Integer, Position> cellTypes = new Hashtable<Integer, Position>(
			100);
	...
	private void drawMazeCell(Position pos, int x, int y, int width, int height,
			Graphics g)
	{
		Position cell = cellTypes.get(maze.getCell(pos));
		if (cell != null)
		{
			g.copyArea(cell.col, cell.row, width, height, x - cell.col,
					y - cell.row);
			numFound++;
		}
		else
	...
	private void drawMaze(Graphics g)
	{
		...
		for (int draw_y = cellHeight + 1; curr.row < maze
				.getHeight(); curr = curr
						.move(Direction.SOUTH), draw_y += cellHeight)
		{
			curr = new Position(0, curr.row);
			for (int draw_x = 2; curr.col < maze.getWidth(); curr = curr
					.move(Direction.EAST), draw_x += cellWidth)
			{
		...
		g.fillRect(maze.getWidth() / 2 * cellWidth + 1, 0, cellWidth - 1,
				cellHeight + 1);
		curr = new Position(maze.getWidth() / 2, maze.getHeight() - 1);
		g.setColor(lookupColor(maze.getColor(curr)));
		g.fillRect(maze.getWidth() / 2 * cellWidth + 1,
				(maze.getHeight() + 1) * cellHeight, cellWidth - 1,
				cellHeight + 1);
